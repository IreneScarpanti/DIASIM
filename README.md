...
`comparison_results.csv`, `election_results.csv`. The `plot_results.py` scripts
under `cmd/` are for exploratory inspection of these CSVs; the figures that appear
in the thesis are separate TikZ/pgfplots sources generated from the same data (see
the thesis figures and their provenance notes).
